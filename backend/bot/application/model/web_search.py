...
class WebSearchRun(Base):
    ...
    message_id: Mapped[str] = mapped_column(ForeignKey("chat_messages.id"), index=True, nullable=False)
...
class WebSource(Base):
    ...
    id: Mapped[str] = mapped_column(String(64), primary_key=True, default=lambda: uuid.uuid4().hex)
    run_id: Mapped[str] = mapped_column(ForeignKey("web_search_runs.id"), index=True, nullable=False)
    user_id: Mapped[int] = mapped_column(ForeignKey("user_details.id"), index=True, nullable=False)
    thread_id: Mapped[str] = mapped_column(ForeignKey("chat_threads.id"), index=True, nullable=False)
    message_id: Mapped[str] = mapped_column(ForeignKey("chat_messages.id"), index=True, nullable=False)
